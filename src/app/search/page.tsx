...
function SearchInner() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      setLoading(true)
      if (!q) { if (on) { setProducts([]); setLoading(false) } ; return }
      const base = process.env.NEXT_PUBLIC_SUPABASE_URL
      const key = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
      const enc = encodeURIComponent(`%${q}%`)
      try {
        const res = await fetch(
          `${base}/rest/v1/products?select=id,name,price_cents,compare_price_cents,stock_qty,images,seller_id,sellers(store_name)&status=eq.active&name=ilike.${enc}&order=created_at.desc`,
  ...
  return (
    <div style={{ fontFamily: 'var(--font-dm-sans)', background: C.offWhite, color: C.g800, minHeight: '100vh' }}>
      <div style={{ background: C.red, padding: '0 20px', minHeight: 60, display: 'flex', alignItems: 'center' }}>
        <div style={{ maxWidth: 1320, margin: 'auto', width: '100%', display: 'flex', alignItems: 'center', gap: 16, padding: '10px 0' }}>
          ...
          <div style={{ flex: 1, display: 'flex', maxWidth: 680, borderRadius: 10, overflow: 'hidden' }}>
            <input value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') submit() }} placeholder="Search products…"
              style={{ flex: 1, border: 'none', padding: '0 16px', fontSize: 14, outline: 'none', height: 40 }} />
            <button onClick={submit} style={{ background: C.navy, border: 'none', color: '#fff', padding: '0 18px', fontSize: 15, cursor: 'pointer' }}>🔍</button>
          </div>
        </div>
      </div>
      <div style={{ maxWidth: 1320, margin: 'auto', padding: '24px 20px' }}>
        <h1 style={{ fontSize: 22, color: C.navy, marginTop: 0 }}>{q ? <>Results for “{q}”</> : 'Search'}</h1>
        {loading ? <p style={{ color: C.g400 }}>Searching…</p>
          : !q ? <p style={{ color: C.g600 }}>Type something in the box above to search.</p>
          : products.length === 0 ? <p style={{ color: C.g600 }}>No products matched “{q}”. Try a different word.</p>
